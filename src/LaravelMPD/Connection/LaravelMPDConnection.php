<?php namespace LaravelMPD\Connection;
/**
* MPDConnection.php: A class for opening a socket connection to MPD
*/

use Config;
use LaravelMPD\Exception\LaravelMPDConnectionException as LaravelMPDConnectionException;

/**
* This class is responsible for establishing a socket connection to MPD
* @package LxMPD
*/
class LaravelMPDConnection {
	...
	private $_properties = array();
		...
		if( !$this->socket = @fsockopen( $this->host, $this->port, $errn, $errs, 5 ) ) {
			...
			throw new LaravelMPDConnectionException( 'Connection failed: '.$errs, self::MPD_CONNECTION_FAILED );
		}
				...
				// Throw an exception and include the response errors
                                throw new LaravelMPDConnectionException( 'Connection failed: '.$matches[4], self::MPD_CONNECTION_FAILED );
                        }
                }
      
		// Throw a general connection failed exception 
		throw new LaravelMPDConnectionException( 'Connection failed', self::MPD_CONNECTION_FAILED );
        }
		...
		try {
			...
			throw new LaravelMPDConnectionException( 'Disconnection failed: '.$e->getMessage(), self::MPD_DISCONNECTION_FAILED );
		}
